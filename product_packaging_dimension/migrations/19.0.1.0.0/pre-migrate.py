...
def _get_uom_from_param(env, param, fallback_xmlid):
    """Return the configured target UoM, falling back to UoM defaults."""
...
def _convert_dimensions_to_target_length_uom(env):
    ...
    In 19.0, ``product_logistics_uom`` makes length a global configuration.
...
def _convert_weight_to_target_weight_uom(env):
    ...
    while ``product_logistics_uom`` supplies the target configured UoM. Convert
    legacy per-record weight values before ``weight_uom_id`` disappears.
    """
