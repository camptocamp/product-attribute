    ...
    def _update_uom(self, uom_id):
        ...
        for key, products_group in groupby(self, key=lambda r: r.uom_id):
            ...
            if (
                ...
                self.env.cr.execute(
                    "UPDATE product_template SET uom_id = %(uom)s WHERE id in \
                     %(product_id)s",
    ...
    def _update_uom_po(self, uom_po_id):
        ...
        for key, products_group in groupby(self, key=lambda r: r.uom_po_id):
            ...
            if (
                ...
                self.env.cr.execute(
                    "UPDATE product_template SET uom_po_id = %(uom)s WHERE id \
                    in %(product_id)s",
                ...
                raise UserError(
                    _(
                        "You can not change the purchase unit of measure of a \
                        product to a new unit that doesn't have the same \
                        category and factor"
                    )
                )
