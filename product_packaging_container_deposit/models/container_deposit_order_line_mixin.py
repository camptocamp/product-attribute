...
class OrderLineMixin(models.AbstractModel):
    ...
    def _get_order_lines_container_deposit_quantities(self):
        """
        Returns a dict with quantity of container deposit
            {
                container_deposit_product: [quantity, qty_dlvd_rcvd]
            }
        """
        ...
        for line in self:
            ...
            line_deposit_dlvd_rcvd_qties = (
                line.product_id.get_product_container_deposit_quantities(
                    line[self._get_product_qty_dlvd_rcvd_field()],
                    forced_packaging=line.product_packaging_id,
                )
            )
    ...
    def write(self, vals):
        ...
        if (
            not self.env.context.get("skip_update_container_deposit")
            or self._get_product_qty_dlvd_rcvd_field() in vals
        ):
