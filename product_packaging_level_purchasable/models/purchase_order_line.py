...
class PurchaseOrderLine(models.Model):
    ...
    def _can_be_purchased_error_condition(self):
        self.ensure_one()
        return self.product_packaging_id and not self.product_packaging_id.purchases
            ...
            if line._can_be_purchased_error_condition():
                raise ValidationError(
                    _(
                        ...
                        " set as 'Purchases' in order to be used on a purchase order."
                    )
                    % {
        ...
        if self._can_be_purchased_error_condition():
            return {
                "warning": {
                    "title": _("Warning"),
                    "message": _(
                        "This product packaging must be set as 'Purchases' in"
    ...
    def _compute_product_packaging_id(self):
        ...
        for line in self:
            if line.product_packaging_id and not line.product_packaging_id.purchases:
