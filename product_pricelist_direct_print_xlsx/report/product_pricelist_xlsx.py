...
class ProductPricelistXlsx(models.AbstractModel):
    ...
    def _add_extra_header(self, book):
        # TODO drop this, this isn't how we should insert extra infos
        # in the document
        return None
    ...
    def _get_header_values(self, book):
        ...
        extra_header = self._add_extra_header(book)
        if extra_header:
            res.append(extra_header)
    ...
    def _get_row_values(self, book, product, formats):
        ...
        values = [(product.display_name, default_format), ]
        extra_info = self._add_extra_info(book, product, **formats)
        # extra info might be an empty string, 0 or FALSE (which are valid xlsx values)
