...
`product.pricelist.item` with different semantics: there the field is computed,
always defaulted to the product base unit, and never used to select a rule.
The incompatibility is declared through the `excludes` manifest key, so Odoo
refuses to install both modules together, whichever is installed first.
